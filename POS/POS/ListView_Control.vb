...
#End Region
    ...
    Friend Sub Load_table(ByVal image_list As ImageList, ByVal list_view As ListView, ByVal table As DataTable, ByVal imageError_Path As Image, ByVal columnName_picture As String, ByVal product_name As String, ByVal statue As String)
        Dim amount_row As Integer = table.Rows.Count - 1
        Dim pic As Image
        Dim img As Byte()
        ...
        For idx As Integer = 0 To amount_row
            Try
                img = table.Rows(idx)(columnName_picture)
                Dim ms As New System.IO.MemoryStream()
                ms.Write(img, 0, img.Length)
                pic = Image.FromStream(ms, True)
                image_list.Images.Add(pic)

            Catch ex As Exception
                image_list.Images.Add(imageError_Path)
            End Try

            Dim name_img As String = table.Rows(idx)(product_name) & " | " & statue
            list_view.Items.Add(name_img, idx)
            list_view.Refresh()
        Next
